...
function getPolynom(...coefficients) {
  ...
  return (x) => {
    ...
    coefficients.forEach((el, i) => {
      result += el * x ** (coefficients.length - i - 1);
    });
...
function logger(func, logFunc) {
  return function fn(...args) {
    const strArg = args.map((arg) => JSON.stringify(arg)).join(',');
